...
@timeit(logger_name='test_logger')
def cross_validate(rank, world_size, test_func, parallel=True):
    """
    ...
    :param test_func: Function which runs training.
    ...
    args.split_keys = ['4foldcv_0'] # List of split keys for cross-validation folds
    args.model_idx = 17
    ...
    for index in range(len(args.split_keys)):
        ...
        _, _, test_adata, ctrl_len = train_valid_test(adata, split_key=split_key, sample=args.data_sample)
        ...
        model_scores = test_func(args, data, logger, split_key)
